...
function SoapCard({
  ...
  const isFav = isInWishlist(Number((soap as any).productId || soap.id));

  return (
    <motion.a
      href={`#product-${soap.id}`}
      ref={productRef}
      ...
      aria-label={`View details for ${soap.name}`}
    >
      <motion.div
      ...
      >
        <img
          ...
          className={`w-full h-full object-cover object-center transition-all duration-700 ease-out group-hover:scale-110 ${soap.images?.[1] ? 'group-hover:opacity-0' : ''} brightness-95 group-hover:brightness-[1.02]`}
        />
        {soap.images?.[1] && (
          <img
            src={soap.images[1]}
            alt={`${soap.name} alternate`}
            loading="lazy"
            decoding="async"
        ...
        {(!soap.variants || soap.variants.length <= 1) && (
          <button
            ...
            aria-label={isFav ? `Remove ${soap.name} from Wishlist` : `Add ${soap.name} to Wishlist`}
          >
  ...
  return (
    <div ref={pageRef} className="relative w-full bg-[var(--color-cream)] text-[var(--color-dark-text)] overflow-hidden font-sans">
      <InnerPageBanner
        ...
        bgImage="/Image/Bannerimg/HomeMadeSoap.webp"
        decorativeEmoji="🌸"
      />

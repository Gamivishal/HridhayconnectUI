...
export function TeaMasalaCategoryPage() {
  ...
  const maxLimit = teaMasalaList.length > 0 ? Math.max(100, Math.ceil(Math.max(...teaMasalaList.map(t => Number(t.sellPrice ?? t.price) || 0)))) : 1000;

  const filteredTeaMasala = teaMasalaList.filter(item => {
          ...
          <div className="w-full lg:w-3/4 xl:w-4/5">
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                {filteredTeaMasala.map((item, index) => (
                  <TeaMasalaCard
                    key={item.id}
                    item={item}
                    index={index}

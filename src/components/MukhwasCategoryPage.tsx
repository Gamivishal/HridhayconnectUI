...
function MukhwasCard({
  ...
  const renderPrice = () => {
    if (item.variants && item.variants.length > 1) {
      const prices = item.variants.map((v: any) => v.sellPrice ?? v.price);
  ...
  const isFav = isInWishlist(Number(item.productId || item.id), Number(item.variants?.[0]?.varientId || item.variantId || 0), packaging);
    ...
    >
      ...
      >
        <img
          ...
          className={`w-full h-full object-cover object-center transition-all duration-700 ease-out group-hover:scale-110 ${item.images?.[1] ? 'group-hover:opacity-0' : ''} brightness-95 group-hover:brightness-[1.02]`}
        />
        {item.images?.[1] && (
          <img
            src={item.images[1]}
        ...
        {(!item.variants || item.variants.length <= 1) && (
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              toggleWishlist(Number(item.productId || item.id), Number(item.variants?.[0]?.varientId || item.variantId || 0), packaging);
            }}
...
export function MukhwasCategoryPage() {
  ...
  const maxLimit = mukhwasList.length > 0 ? Math.max(100, Math.ceil(Math.max(...mukhwasList.map(m => Number(m.sellPrice ?? m.price) || 0)))) : 1000;

  const filteredMukhwas = mukhwasList.filter(item => {
          ...
          <div className="w-full lg:w-3/4 xl:w-4/5">
            <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
              <div>
                ...
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif text-black leading-tight font-light">
                  Hand-Roasted <br />
                  <span className="italic text-[var(--color-secondary)]">Mukhwas Masterpieces</span>
                </h2>
              </div>
            ...
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-32 text-[var(--color-primary)]">
                <Sparkle className="w-10 h-10 animate-spin" />
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                {filteredMukhwas.map((item, index) => (
                  <MukhwasCard
                    key={item.id}
                    item={item}
                    index={index}

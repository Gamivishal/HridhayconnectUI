...
function HairOilCard({
  ...
  const isFav = isInWishlist(Number((oil as any).productId || oil.id));
  ...
  return (
    <motion.a
      ...
      aria-label={`View details for ${oil.name}`}
    >
      <motion.div
      ...
      >
        ...
        {(!oil.variants || oil.variants.length <= 1) && (
          <button
            ...
            aria-label={isFav ? `Remove ${oil.name} from Wishlist` : `Add ${oil.name} to Wishlist`}
          >
...
export function HairOilCategoryPage() {
    ...
    async function loadProducts() {
      try {
        ...
        if (isMounted) {
          ...
          if (cardOils.length > 0) {
            const maxVal = Math.max(100, Math.ceil(Math.max(...cardOils.map(o => Number(o.sellPrice ?? o.price) || 0))));
            setMaxPrice(maxVal);
          }
  ...
  const maxLimit = oilsList.length > 0 ? Math.max(100, Math.ceil(Math.max(...oilsList.map(o => Number(o.sellPrice ?? o.price) || 0)))) : 1000;

  const filteredOils = oilsList.filter(oil => {
    const price = oil.sellPrice ?? oil.price;
